  ...
  grafanaDashboards+::
    {
      ...
      'loki-deletion.json': {
        compactor_matcher:: 'pod=~"(compactor|%s-backend.*|loki-single-binary)"' % $._config.ssd.pod_prefix_matcher,
        ...
        .addPanel(
          ...
          g.queryPanel('node_namespace_pod_container:container_cpu_usage_seconds_total:sum_irate{%s, %s}' % [$.namespaceMatcher(), dashboards['loki-deletion.json'].compactor_matcher], '{{pod}}'),
        )
        .addPanel(
          $.newQueryPanel('Compactor memory usage (MiB)') +
          g.queryPanel('go_memstats_heap_inuse_bytes{%s, %s} / 1024 / 1024 ' % [$.namespaceMatcher(), dashboards['loki-deletion.json'].compactor_matcher], ' {{pod}} '),
        )
        .addPanel(
        ...
        .addPanel(
          ...
          g.queryPanel('sum(rate(loki_compactor_deleted_lines{' + $.namespaceMatcher() + ', ' + dashboards['loki-deletion.json'].compactor_matcher + '}[$__rate_interval])) by (user)', '{{user}}'),
        )
      ).addRow(
        g.row('List of deletion requests')
        .addPanel(
          $.logPanel('In progress/finished', '{%s, %s} |~ "Started processing delete request|delete request for user marked as processed" | logfmt | line_format "{{.ts}} user={{.user}} delete_request_id={{.delete_request_id}} msg={{.msg}}" ' % [$.namespaceMatcher(), dashboards['loki-deletion.json'].compactor_matcher]),
        )
        .addPanel(
          $.logPanel('Requests', '{%s, %s} |~ "delete request for user added" | logfmt | line_format "{{.ts}} user={{.user}} query=\'{{.query}}\'"' % [$.namespaceMatcher(), dashboards['loki-deletion.json'].compactor_matcher]),
        )
      ),
    },
}
